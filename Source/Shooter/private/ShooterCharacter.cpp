...
float AShooterCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstgator, AActor* DamageCauser)
{
	if (Health - DamageAmount <= 0.f)
	{
		...
		auto EnemyController = Cast<AEnemyController>(EventInstgator);
		if (EnemyController)
		{
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	{
		// OutBaemLocation is the End Location for the line trace
	}
...
void AShooterCharacter::ExchangeInventoryItems(int32 CurrentItemIndex, int32 NewItemIndex)
{
	const bool bCanExchangeItems =
		...
		(CombatState == ECombatState::ECS_Uncoccupied || CombatState == ECombatState::ECS_Equipping);
